...
sub parse($$) {
   ...
   given ($txt) {
      ...
      when (/\bплазм(?:[ауы]|ой)\b/i) {
         
         return (1, 0, "Плазма не падает!") if int(2*rand);
      }

      when (/\b2007(?:[мй]|ого|ому)?\b/i) {
      ...
      when (/\bкожан+(?:ы[хме]|ыми|ого)\s+(?:ублюдк|мешк)(?:[иа]|ами|ов?)/i) {
         ...
         return (1, 0, $t) unless int(5*rand);
      }
